...
document.getElementById("btnRegistrarse").addEventListener("click", (e) => {
  ...
  const validationEmailRegistered = storageParsed.filter(
    (element) => element.email === email_usuario
  );

  if (validationEmailRegistered.length > 0) {
    swal(
      `Please enter another email address, ${email_usuario} already in use`,
      "",
      "warning"
    );
    return;
  ...
  if (name_usuario === "" || email_usuario === "" || password_usuario === "") {
    ...
    swal("Good Job!", "The account was successfully created", "success");
    $signUp.classList.toggle("active");
    $signIn.classList.toggle("active");
  }
...
document.getElementById("btnInicionSesion").addEventListener("click", (e) => {
  ...
  storageParsed.forEach((element) => {
    const emailValidated =
      element.email.toUpperCase() === ingreso_email.toUpperCase() ?? false;
    const passwordValidated =
      element.password.toUpperCase() === ingreso_password.toUpperCase() ??
      false;
